// /Users/nick/NiCode GmbH/Pommerel/supabase/functions/send-email/index.ts
...
Deno.serve(async (req) => {
  try {
    ...
    // Query messages that are due to be sent (status='scheduled' and due <= now)
    // Now includes campaigns table join to get sender email from campaign instead of message
    const { data: dueMessages, error: queryError } = await supabase
      ...
      .select(`
        ...
        message,
        sender,
        leads!inner(email, campaign_id, company_website, status),
        campaigns!inner(email)
      `)
      .eq('status', 'scheduled')
    ...
    for (const msg of dueMessages) {
      try {
        ...
        const unsubscribeUrl = `https://unsubscribe.resend.com/unsubscribe?campaign_id=${msg.campaign_id}&company_website=${encodeURIComponent(msg.leads.company_website || '')}`
        
        // Add DSGVO footer to message content only if lead status is 'active'
        let emailContent = msg.message
        if (msg.leads.status === 'active') {
        ...
        const senderDomain = msg.campaigns.email.split('@')[1] || 'example.com'
